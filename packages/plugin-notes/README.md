...
- **client**
  ...
  - 便签板浮层（shell.overlay）：**行式列表 / grid 纸卡墙**双视图（Win11 便签式
    六色纸卡，置顶优先）切换 + **按颜色多选筛选** + 底部**归档折叠区**（默认收起，
    展开才渲染），新建/编辑走 tiptap **Markdown** 编辑器（底部选便签纸色）
  ...
  - client 结构：`views/`（页面：入口/浮层/设置卡片）、`components/`（复用组件：
    纸卡/行/色筛/归档折叠/编辑器等）、`core/`（状态/远程通道/纯函数/工具）
