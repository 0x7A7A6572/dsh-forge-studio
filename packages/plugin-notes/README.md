...
这一行还捎带**待办数**与**快捷新建 (＋)**，两侧由一根弹簧撑开，渲染出来就是整行
`[图标 便签 ......... 待办数 (＋)]`（有待办才显示数字；点 (＋) 只弹快捷新建，不会连带打开便签板）。
它是插件**接管**宿主那一行做出来的：槽位只给一个字形位、标题由侧栏自己渲染在字形之后，
所以标题与控件都长在字形里，再靠 CSS 把宿主标题藏掉（判定与失效回退见
`src/client/styles/notes-entry.module.css`）。
...
**打开方式**（「入口」分组顶上那个下拉）管的是板子**开在哪**：默认「中间列」——也就是插件一直以来的行为；
也可以改成「右侧栏」，落点与导引卡片点进去的是同一个 tab，打开时右侧栏会自动展开。宿主没装右侧栏时
自动回退中间列。这个设置只管左栏顶部那一行与输入栏的「打开便签板」；导引页那张卡片照旧，它本来就在右侧栏。

四个入口**默认全开**，可以逐项关掉。但**至少要留一个**：全关掉之后便签板自己就没有打开的地方了
（设置页在 dsh 设置里，不受入口开关影响），所以设置页会把最后一个开着的入口锁住，不让你关。
...
**基础**（默认标题 / 默认工作区）、**入口**（打开方式 + 各入口开关）、**备份**（WebDAV）。
...
| `openMode` | 便签板的打开方式：`main` 中间列（默认）/ `right` 右侧栏 |
| `entry` | 各会话入口的开关（dsh 设置 → 便签 → 入口） |
| `webdav` | WebDAV 备份配置：`enabled` / `url` / `username` / `password` / `path` / `intervalMin` / `keep` |

## 备份（WebDAV）

**dsh 设置 → 便签 → 备份**里填服务器地址、远端目录、账号与**应用密码**（凭据只存本机设置，
建议用服务商提供的应用密码而非主密码）。按**检查间隔**定时比对，**只在便签有变更时**上传快照，
远端只留最近**保留份数**份（超出删最旧）。想立刻验证连通可以点「保存并立即备份」；
恢复前会先自动备份当前状态，选中的那份**整体覆盖**现有便签。
