 ...
 * 泳道「执行」照常把任务投给 agent——“便签以 notes_* 工具暴露”变成空头支票：
 * 执行会话按协议调用 notes_task_* 只会撞 unknown tool（泳道任务执行协议全线
 * 失败，即「工具注册好像有问题」bug）。现在把桥接结果提为显式状态机
 ...
 * - failed 时以 logger.error 级别记录原因（旧实现仅 warn），host 可查、可透出
 *   给 client 渲染，避免再次出现“工具挂了却无信号”的静默失败。
 *
