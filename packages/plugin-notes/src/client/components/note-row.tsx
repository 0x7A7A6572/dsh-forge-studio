/**
 * 便签行（列表视图）：一行一条便签 —— 左侧纸色小方块 + 标题/单行摘要 +
 * 更新时间 + 悬停浮出的操作（编辑/置顶/归档或恢复/删除）。
...
.fs-note-row .fs-note-actions button[data-danger]:hover:not(:disabled) { background: var(--dsw-alias-interactive-bg-hover-danger); color: var(--dsw-alias-state-error-primary); }
`
  ...
  return (
    <li>
      <div
        ...
        style={rowStyle}
      >
        <span style={{ ...colorDot, background: meta.paper, ...(note.pinned && !note.archived ? { boxShadow: `0 0 0 2px ${meta.ring}` } : {}) }} />
        ...
        <time style={timeStyle} title={new Date(note.updatedAt).toLocaleString()}>{fmtRelative(note.updatedAt)}</time>
