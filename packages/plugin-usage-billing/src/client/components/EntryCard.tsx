...
export function EntryCard(props: EntryCardProps): JSX.Element | null {
  const { load, failed, amountText, headlineText, todayText, budgetBar, unpricedText, segments, seat, ...rest } =
    useEntryCard(props)
  const visible = useEntryVisible(props.scope, 'sidebar')
  if (!visible) return null

  return (
    <span className={styles.entrySeat + ' ' + styles.palette} ref={seat.anchorRef}>
      <button
        type="button"
      ...
      >
        {props.wide ? null : (
          ...
          <span className={styles.entryIcon} data-dsh-ub-icon aria-hidden="true">
            <PieBadge ratio={budgetBar?.ratio ?? null} level={budgetBar?.level ?? 'ok'} size={18} />
          </span>
        )}
        <span className={styles.entryText} data-dsh-ub-entry-text>
          <span className={styles.entryLine}>
            ...
            <span className={styles.entryToday} data-dsh-ub-today>
              <span className={styles.dot} data-kind="today" aria-hidden="true" />
              {`今 ${todayText}`}
            </span>
          </span>
          {budgetBar === null ? null : (
