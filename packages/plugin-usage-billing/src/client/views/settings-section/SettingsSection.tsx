...
export function SettingsSection(props: SettingsSectionProps): JSX.Element {
  const {
    ...
    view, setView, tab, setTab, visitedTabs,
    activityOpen, modelsOpen, toggleActivity, toggleModels,
    budgetNotice, dismissBudgetNotice, dismissBackfill, backfillDismissed,
  ...
  const onTabKeyDown = (event: KeyboardEvent<HTMLButtonElement>, index: number): void => {
    let next: number
    switch (event.key) {
      case 'ArrowLeft': next = (index + SETTINGS_TABS.length - 1) % SETTINGS_TABS.length; break
      case 'ArrowRight': next = (index + 1) % SETTINGS_TABS.length; break
