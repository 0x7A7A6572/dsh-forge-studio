...
/** 四格等宽指标（累计卡与今日卡同规格）。 */
function cells(t: Pick<DailyPoint, 'input' | 'cacheRead' | 'output' | 'calls'>): ReadonlyArray<{ label: string; value: string }> {
  return [
  ...
  return (
    <div className={styles.section} data-dsh-usage-billing>
            ...
            {overview.unpricedModels.length > 0 ? (
              <>
                ...
                {/* 分隔号放在 span **外面**：读屏（与测试）按元素全文取名字，'
                    · 1 未收录' 和 '1 未收录' 是两回事。 */}
                <span className={styles.unpriced}>{overview.unpricedModels.length + ' 未收录'}</span>
              </>
            ) : null}
            {/* 标记与金额同源（同一次 overview 响应）；缺席按 present 处理，与另外三个分区同一保守口径。 */}
            {backfilledDisclosure(overview.hasBackfilled)
              ? <span className={styles.estimate} data-dsh-ub-estimate> · 含安装前估算</span>
              : null}
          </>
        }
        footnote={budget.enabled ? (
          <>
            <div className={styles.barMeta}>
              ...
              {spend.pct >= 1
                ? <span className={styles.danger}>超支 {formatCny(monthSpend - budget.monthlyCny)}</span>
                : null}
            </div>
            <ProgressBar
              level={spend.level}
      ...
      <Card
        title="最近活跃度"
        extra={(
          <div className={styles.tabs} role="group" aria-label="活跃度时间窗">
            {HEAT_WINDOWS.map((w) => (
