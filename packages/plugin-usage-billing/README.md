...
- 侧栏入口卡（本月费用 + 今日 + 预算进度条 + 7 天 sparkline，折叠 rail 退化为图标钮）；点击打开全屏仪表盘（宿主 `Modal`，portal 到 body），含概览 / 趋势 / 热力图 / 明细 / 费率五个分区。侧栏刻意**不**显示日期、价表来源、未收录计数与估算角标 —— 这四样都有更合适的落点（弹窗 / 概览页 / 设置页），挤在 56px 宽的卡片里只会变成噪音。
