...
| **记忆图谱** | 「更多 ▾ → 记忆图谱」打开一张可拖动、可缩放的关系图：节点是记忆加全部实体，连线是它们之间真实存在的关系（自动生成的共现边画成虚线）。弹窗里可直接在**全局 / 项目**之间切换（选项目时还能挑具体工作区），不用退出去换页签。悬停看细节，点记忆节点开它的详情——详情浮在图谱上，图谱不关；详情里「关联」标题与每条关联右侧都有定位按钮，点了跳去图谱点亮那个节点（记忆会顺带把图谱范围切到它所在）。配色跟着深浅主题走。记忆超过 500 条时只画关联最多的那些，并在图上方注明省略了几条。 |
